...
# MOLEBERT_CODE_DIR = "/share/home/qiujh/science/tools/DLenzyme/202606/5_infer/utils"
...
def main():
    ...
    # parser.add_argument('--pre_trained_model_path', type=str, default='/lustre/home/jhqiu/tools/retrain/sakpe/Mole-BERT/data/model_gin/Mole-BERT.pth', help='预训练模型路径')
    ...
    # /lustre/home/jhqiu/tools/retrain/sakpe/Mole-BERT/data_retry_3/dataset/v3-2_all
    # parser.add_argument('--input_csv_path', type=str, default='/lustre/home/jhqiu/tools/retrain/sakpe/Mole-BERT/data_retry_4/dataset/v3-2_all', help='数据集根路径')
    # parser.add_argument('--input_csv_path', type=str, default='/share/home/qiujh/science/tools/DLenzyme/sakpe_20250529/3_substrate_feature_2507/mole_bert/data/dataset/v3-2_all_retrain/', help='数据集根路径')
    parser.add_argument(
        '--input_csv',
        type=str,
        required=True,
    ...
    # /lustre/home/jhqiu/tools/retrain/sakpe/Mole-BERT/data_retry_3/Smi_molebert_lmdb
    # parser.add_argument('--output_lmdb_path', type=str, default='/lustre/home/jhqiu/tools/retrain/sakpe/Mole-BERT/data_retry_4/Smi_molebert_lmdb', help='数据集根路径')
    ...
    # working_path = "/lustre/home/jhqiu/tools/retrain/sakpe/Mole-BERT/data_retry_3"
    # dataset = MoleculeDataset(working_path + "/dataset/" + args.dataset, dataset=args.dataset)  # v3-2_all
    # dataset = MoleculeDataset(args.input_csv_path, dataset=args.dataset)
    ...
    # env = lmdb.open('/lustre/home/jhqiu/tools/retrain/sakpe/Mole-BERT/data_retry_3/Smi_molebert_lmdb', map_size=1073741824)
    ...
    for step, batch in enumerate(tqdm(generate_embedding_loader, desc="进度 = Iteration")):
        ...
        # batch = batch.to(torch.float64)  # 数据转为双精度
        ...
        with env.begin(write=True) as txn:
            for i in range(len(batch.ptr) - 1):
                ...
                id = int(batch.y[i].cpu())
                # 跳过自动补进去的 dummy 分子 SMI_000
                if id == 0:
                    print("[INFO] 跳过 dummy 分子 SMI_000")
                    continue
                # 3. 获得第 i 个图所有节点表示的索引
                # batch.ptr = tensor([0, 4, 9, 12]) = 图 0/1/2 节点占 [0:4) / [4:9) / [9:12) = 3 图(分别含 4/5/3 个节点)
                # batch.ptr[i] = 第 i 条图节点在[ ptr[i] : ptr[i+1] )间 | len = 图 + 1
                ...
                # import io
                # buffer = io.BytesIO()
                # np.save(buffer, graph_feature.cpu().numpy())
