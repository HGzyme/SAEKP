...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        elif self.dataset == 'v3-2_all':
            input_path = os.path.join(self.raw_dir, 'smiles_ids.csv')
            csv_file_path = os.path.join(self.raw_dir, 'molebert_failed.csv')
            smiles_list, rdkit_mol_objs, labels = _load_v2_dataset(input_path)
            failed_smiles = []
        # elif self.dataset == 'v3-2_all':
        #     smiles_list, rdkit_mol_objs, labels = \
        #         _load_v2_dataset('/share/home/qiujh/science/tools/DLenzyme/sakpe_20250529/3_substrate_feature/data/dataset/v3-2_all_retrain/raw/smiles_ids.csv')
        #     failed_smiles = []
        #     csv_file_path = '/share/home/qiujh/science/tools/DLenzyme/sakpe_20250529/3_substrate_feature/data/dataset/v3-2_all_retrain/raw/v3_Molebert_failed.csv'
            ...
            for i in range(len(smiles_list)):
                try:
                    ...
                    print(f"Error processing SMILES {smiles_list[i]}")
                    failed_smiles.append({'id': i, 'smiles': smiles_list[i], 'error': str(e)})
        
                    # 将失败的信息写入 CSV 文件
                    with open(csv_file_path, mode='a', newline='') as file:
                        writer = csv.writer(file)
                        writer.writerow([i, smiles_list[i]])
...
def _load_v2_dataset(input_path):
    """
    :param input_path:
    :return: list of smiles, list of rdkit mol obj, np.array containing the
    labels
    """
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['isomeric_smiles']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    # labels = input_df['id']
    labels = input_df['id'].str.extract(r'(\d+)').astype(int)
    # convert 0 to -1
    labels = labels.replace(0, -1)
    # there are no nans
    assert len(smiles_list) == len(rdkit_mol_objs_list)
